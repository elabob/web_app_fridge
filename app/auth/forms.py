...
from wtforms.validators import InputRequired,DataRequired, Length     #validators  - kontroluju ci je pole vyplnene (DataRequired), a ci maju spravnu dlzku

class LoginForm(FlaskForm):
    username = StringField('Username', validators=[InputRequired(), Length(min=4, max=20)])
    password = PasswordField('Password', validators=[InputRequired(), Length(min=6, max=30)])
    submit = SubmitField('Login')       #tlacitko na odoslanie formulara
